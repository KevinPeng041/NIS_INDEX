...
function GetIOACPageJson($conn,$sPg,$sTraID){
    $TP_SQL="SELECT ST_DATA".$sPg." FROM HIS803.NISWSTP WHERE ID_TRANSACTION=:sTraID AND ID_TABFORM = 'IOA_C'";
    $TP_stid=oci_parse($conn,$TP_SQL);
    if (!$TP_stid){
        $e=oci_error($conn);
        return $e['message'];
...
function PosIOACalssSave($conn,$sTraID,$sUr){
    ...
    $Ssql="SELECT ID_INPATIENT,ID_PATIENT,ST_DATAC,ID_BED,JID_NSRANK,FORMSEQANCE_WT
           FROM HIS803.NISWSTP
           WHERE ID_TABFORM = 'IOA_C'  AND ID_TRANSACTION = :id_TRANS";
