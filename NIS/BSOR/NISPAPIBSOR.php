...
function PosBSORSave($conn, $sTraID, $sFm, $sDt, $sTm, $sUr)
{
    ...
    $obj_B = array_filter(json_decode($ST_DATAB), function ($val) {
    ...
    if (count($Insert_PIXCEL_Data) > 0) {
        ...
        if (count($Cancel_PIXCEL_Data) > 0) {
            foreach ($Cancel_PIXCEL_Data as $value) {
                ...
                if ($sFm == "BSOR" || $sFm == "CUTS") {
                    ...
                    $GetFilter = array_filter($obj_B, function ($val) use ($NUM) {
        ...
        foreach ($Insert_PIXCEL_Data as $value) {
            ...
            $GetData = array_filter($obj_B, function ($val) use ($NO_BEDSORE) {
                return $val->TB_DATA->NO_NUM->VALUE == $NO_BEDSORE;
            });
            sort($GetData);
    ...
    if ($sFm == "BSOR" || $sFm == "CUTS") {
        ...
        foreach ($DTSEQ_SFMSEQ as $key => $value) {
            ...
            //取表單對應的obj
            $TIBS_Obj = array_filter($Insert_TB_DATA, function ($value) use ($FORMSEQANCE_BS) {
                return $value->FORMSEQ != "" && $value->FORMSEQ == $FORMSEQANCE_BS;
            });
        ...
        $Insert_NUM = array_map(function ($val) {
            return $val->NUM;
        }, $Insert_PIXCEL_Data);

        foreach ($Insert_NUM as $NO_NUM) {
            $DATA = array_filter($Insert_TB_DATA, function ($val) use ($NO_NUM) {
                return $val->TB_DATA->NO_NUM->VALUE == $NO_NUM;
            });

            $get_DTSEQ = array_filter($DTSEQ_SFMSEQ, function ($val) use ($NO_NUM) {
                return json_decode($val)->NUM == $NO_NUM;
            });
            sort($get_DTSEQ);
...
function obj_diff($Page, $new_obj, $default_obj)
{
    ...
    if ($Page == "A") {
        ...
        $oldOBJ = array_filter($default_Data, function ($value) {
            return $value->NUM != "";
        });
        $newOBJ = array_filter($new_Data, function ($value) {
            return $value->NUM != "";
        });
        ...
        $oldOBJ = array_filter($default_Data, function ($value) {
            return $value->TB_DATA->NO_NUM->VALUE != "";
        });
        $newOBJ = array_filter($new_Data, function ($value) {
            return $value->TB_DATA->NO_NUM->VALUE != "";
        });
        ...
        if (count($oldOBJ) == 0) {
            ...
            $New_TB = array_map(function ($value) {
                return $value->TB_DATA;
            }, $newOBJ);
            $Default_TB = array_map(function ($value) {
                return $value->TB_DATA;
            }, $oldOBJ);
            ...
            foreach ($New_TB as $value) {
                ...
                //比對是否內含新資料
                $filter_oldObj = array_filter($Default_TB, function ($val) use ($new_Num) {
                    return $val->NO_NUM->VALUE == $new_Num;
                });
